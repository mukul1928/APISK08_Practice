...
public class StepDefinition {
	...
	public void user_enters_the_payload_data() throws JsonProcessingException {
		res = given().log().all().header("x-api-key", "reqres_d8ebbfbe96b941cabc658bf97bd83573")
				.relaxedHTTPSValidation().spec(req).body(CreateUserData.createUser());
	}

	@When("user sumbits the payload with an endpoint as {string}")
	public void user_sumbits_the_payload_with_an_endpoint_as(String endpoint) {
	...
	public void validate_user_gets_created_and_corresponding_status_code_is(String status_code) {
		...
		if (resTime > 5000) {
			throw new ArithmeticException("Longer than expceted time response");
		} else {
	...
	public void user_validates_himself_with_auth_token() {
		authToken = FetchDataFromProperty.readDataFromProperty().getProperty("auth_token");
	}
	...
	public void user_passes_the_payload_with_all_details() throws IOException {
		...
		.body(GetDataExcel.getExcelData());
	}
	...
	@Then("user validate a new user created with status code as {string}")
	public void user_validate_a_new_user_created_with_status_code_as(String status_code) {
		Integer Code = response.getStatusCode();
	...
	@Given("user hits the paypal site")
	public void user_hits_the_paypal_site() {
	...
	public void user_fetches_the_actual_token_from_access_token() {
		RestAssured.baseURI = "https://api-m.sandbox.paypal.com";
		String res = given().log().all().queryParam("page",3).queryParam("page_size", 4).queryParam("total_count_required", true)
